  ...
  if (isAuthenticated && user) {
    switch (user.role) {
      ...
      case UserRole.DRIVER:
        return <Navigate to="/driver/trips" />;
      case UserRole.CLIENT:
        return <Navigate to="/client/request" />;
      default:
        return <Navigate to="/" />;
    }
  }
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      if (err.response?.status === 401) {
        ...
        setError(
          `Ruta no encontrada (404). Verifique la URL de la API. Intentó conectar a: ${err.config?.baseURL}${err.config?.url}`,
        );
      } else {
  ...
  return (
    <div className="bg-gray-50 font-sans min-h-screen flex text-gray-800 selection:bg-secondary selection:text-white">
      ...
      <div className="hidden lg:flex w-full lg:w-5/12 bg-[#001a33] relative justify-center items-center overflow-hidden">
        ...
        <div className="relative z-10 p-12 text-white max-w-lg">
          ...
          <h2 className="text-3xl font-bold mb-6 leading-tight text-white">
            Tu socio confiable en rutas mineras y turismo.
          </h2>
          <p className="text-blue-100 text-lg leading-relaxed mb-8">
            Gestiona tus viajes, monitorea tus rutas y accede a soporte 24/7 a través de nuestra
      ...
      <div className="w-full lg:w-7/12 flex flex-col justify-center items-center p-6 lg:p-20 bg-white relative">
        ...
        <Link
          to="/"
          className="absolute top-6 right-6 lg:top-10 lg:right-10 text-gray-400 hover:text-primary transition flex items-center gap-2 text-sm font-medium group"
        >
        ...
        <div className="w-full max-w-md">
          ...
          <div className="lg:hidden flex justify-center mb-8">
            <div className="w-16 h-16 bg-primary rounded-xl flex items-center justify-center text-white text-3xl font-bold shadow-md">
              EL
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <Link
                  to="/recover-password"
                  className="text-xs font-semibold text-primary hover:text-secondary transition"
                >
          ...
          <div className="mt-8 pt-6 border-t border-gray-100 text-center">
            <p className="text-sm text-gray-500">
              ...
              <Link
                to="/register"
                className="font-bold text-secondary hover:text-orange-700 transition"
              >
                Regístrate aquí
              </Link>
            </p>
