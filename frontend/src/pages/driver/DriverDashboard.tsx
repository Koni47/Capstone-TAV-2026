...
import { BarChart3, MapPin, Car, Wallet, Star, Clock, CheckCircle2 } from 'lucide-react';
import {
  Chart as ChartJS,
  CategoryScale,
  LinearScale,
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => (
          <div
            key={index}
            className={`bg-white rounded-lg shadow-sm p-6 border-l-4 ${stat.border} hover:shadow-md transition`}
          >
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="space-y-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-blue-50/50 border-l-4 border-blue-500 rounded-r-lg hover:bg-blue-50 transition"
            >
              ...
              <div className="mt-2 sm:mt-0 text-right">
                ...
                <span className="inline-block px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded font-bold mt-1">
                  ASIGNADO
                </span>
              </div>
            </div>
