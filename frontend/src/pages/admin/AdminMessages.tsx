...
const AdminMessages = () => {
  ...
  const getReasonBadge = (reason: string) => {
    switch(reason) {
        case "reclamo": return <Badge variant="error" className="uppercase">Reclamo</Badge>;
        case "felicitacion": return <Badge variant="success" className="uppercase">Felicitación</Badge>;
        case "comentario": return <Badge variant="info" className="uppercase">Consulta</Badge>;
        default: return <Badge variant="default" className="uppercase">{reason}</Badge>;
    }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex items-center justify-between">
                <div>
                    ...
                    <p className="text-2xl font-bold text-primary">{messages.filter(m => m.status === 'PENDING').length}</p>
                </div>
                <div className="p-2 bg-blue-50 rounded-lg text-primary"><Mail size={24} /></div>
            </div>
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex items-center justify-between">
                <div>
                    <p className="text-xs font-bold text-gray-400 uppercase">Reclamos</p>
                    <p className="text-2xl font-bold text-red-600">{messages.filter(m => m.reason === 'reclamo').length}</p>
                </div>
                <div className="p-2 bg-red-50 rounded-lg text-red-600"><Star size={24} /></div>
            </div>
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex items-center justify-between">
                <div>
                ...
                <div className="p-2 bg-gray-50 rounded-lg text-gray-600"><MessageSquare size={24} /></div>
            </div>
        </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Remitente</th>
                            <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Motivo</th>
                            <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Mensaje</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {messages.length === 0 ? (
                            ...
                            messages.map((msg) => (
                                <tr key={msg.id} className={`hover:bg-gray-50 transition cursor-pointer ${msg.status === 'PENDING' ? 'bg-blue-50/30' : ''}`}>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <div className="flex flex-col">
                                            <span className="font-semibold text-gray-900">{format(msg.date, "dd MMM", { locale: es })}</span>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-xs text-gray-500 flex items-center gap-1"><Mail size={10}/> {msg.email}</span>
                                            <span className="text-xs text-gray-500 flex items-center gap-1"><Phone size={10}/> {msg.phone}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td className="px-6 py-4">
                                        <p className="text-sm text-gray-600 line-clamp-2 max-w-xs" title={msg.comment}>
                                            {msg.comment}
                                        </p>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-center">
                                        {msg.rating > 0 && (
                                            <div className="flex items-center justify-center gap-1 text-[#FF6600]">
                                                <span className="font-bold text-sm">{msg.rating}</span> <Star size={14} className="fill-[#FF6600]" />
                                            </div>
                                        )}
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div className="flex items-center justify-end gap-2">
                                            <button className="p-1.5 hover:bg-blue-100 text-blue-600 rounded-md transition" title="Ver detalle">
                                                <Eye size={18} />
                                            </button>
                                            <button 
