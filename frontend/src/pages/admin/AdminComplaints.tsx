...
import { Trash2, Eye, CheckCircle, Filter, ShieldAlert, Archive } from 'lucide-react';
...
const AdminComplaints = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'PENDING':
        return <Badge variant="error">Pendiente</Badge>;
      case 'INVESTIGATING':
        return <Badge variant="warning">En Investigación</Badge>;
      case 'CLOSED':
              ...
              {complaints.map((complaint) => (
                <tr key={complaint.id} className="hover:bg-blue-50/50 transition duration-150">
                  <td className="p-4 whitespace-nowrap">
                    <span className="block font-mono font-bold text-primary text-sm">
                      {complaint.id}
                    </span>
                  ...
                  <td className="p-4">
                    ...
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded border ${getPriorityColor(complaint.priority)}`}
                    >
