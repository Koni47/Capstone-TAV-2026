...
const DispatchPage = () => {
  ...
  const confirmAssignment = () => {
    ...
    alert(
      `Solicitud ${selectedRequestId} asignada al conductor ${selectedDriverId} correctamente.`,
    );
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      case 'AVAILABLE':
        return { bg: 'bg-green-100', text: 'text-green-800', label: 'Disponible' };
      case 'ON_ROUTE':
        return { bg: 'bg-blue-100', text: 'text-blue-800', label: 'En Ruta' };
      case 'MAINTENANCE':
        return { bg: 'bg-yellow-100', text: 'text-yellow-800', label: 'Mantención' };
      default:
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Monitoreo en tiempo real y asignación de solicitudes.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {drivers.map((driver) => {
          ...
          return (
            <div
              key={driver.id}
              className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition duration-300 group"
            >
              {/* Card Header */}
                ...
                >
                  ...
                  {requests.map((req) => (
                    <option key={req.id} value={req.id}>
                      {req.id} - {req.origin.substring(0, 15)}... ➔{' '}
                      {req.destination.substring(0, 15)}...
                    </option>
                  ))}
                </select>
              </div>
